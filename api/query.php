...
if ($type === 'balance') {
    ...
    if ($data['code'] !== '0000') {
        echo json_encode(['success' => false, 'message' => '查询失败: ' . ($data['msg'] ?? '未知错误')], JSON_UNESCAPED_UNICODE);
        exit;
    }
...
function sendNotification($stats, $notifyConfig, $lastStats) {
    ...
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode([
        ...
        'title' => $title,
        'subtitle' => $subtitle,
        'content' => $content  // 只发送content，不重复subtitle
    ]));
